...
class CubeLevel extends Level {
  ...
  _validateWithParentLevel(parentLevel) {
    ...
    if (numHorizontal % parentNumHorizontal !== 0) {
      throw new Error(
        `Number of horizontal tiles must be multiple of parent level: ${numHorizontal} (${width}/${tileWidth}) vs. ${parentNumHorizontal} (${parentWidth}/${parentTileWidth})`
      );
    }

    if (numVertical % parentNumVertical !== 0) {
      throw new Error(
        `Number of vertical tiles must be multiple of parent level: ${numVertical} (${height}/${tileHeight}) vs. ${parentNumVertical} (${parentHeight}/${parentTileHeight})`
      );
    }
  }
}
...
CubeGeometry.Tile = CubeTile;
CubeGeometry.type = 'cube';
CubeTile.type = 'cube';
